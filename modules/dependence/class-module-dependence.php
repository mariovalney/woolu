...
if ( ! class_exists( 'WOOLU_Module_Dependence' ) ) {

    class WOOLU_Module_Dependence {
        ...
        public function admin_notices() {
            foreach ( $this->notices as $notice ) {
                $notice = $notice;
                include_once WOOLU_PLUGIN_PATH . '/modules/dependence/includes/views/html-notice.php';
            }
        }
        ...
        private function create_install_plugin_notice( $plugin ) {
            ...
            return sprintf(
                ...
                __( '<strong>WooLu</strong> depends of %1$s to work. Click to %2$s.', 'woolu' ),
                $plugin->name,
                '<a href="' . esc_url( $url ) . '">' . __( 'install the plugin', 'woolu' ) . '</a>'
            );
        }
        ...
        private function create_activate_plugin_notice( $plugin ) {
            ...
            return sprintf(
                ...
                __( '<strong>WooLu</strong> depends of %1$s to work. Click to %2$s.', 'woolu' ),
                $plugin->name,
                '<a href="' . esc_url( $url ) . '">' . __( 'activate the plugin', 'woolu' ) . '</a>'
            );
        }
