...
/**
 * Plugin Name:     WooLu
 * Plugin URI:      https://github.com/mariovalney/woolu
 * Description:     Integrate your store to Magalu!
 * Version:         1.0.0
    ...
    class WooLu {
        ...
        private function add_modules() {
            ...
            foreach ( $results as $result ) {
                ...
                $classname = 'WOOLU_Module_' . $classname;
        ...
        public function load_plugin_textdomain() {
            load_plugin_textdomain( 'woolu', false, basename( dirname( __FILE__ ) ) . '/languages/' );
        }
        ...
        public function run() {
            ...
            define( 'WOOLU_VERSION', '1.0.0' );
            define( 'WOOLU_PLUGIN_FILE', __FILE__ );
            define( 'WOOLU_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );
            define( 'WOOLU_PLUGIN_PATH', WP_PLUGIN_DIR . '/' . dirname( WOOLU_PLUGIN_BASENAME ) );
            ...
            foreach ( $this->modules as $module_slug => $module ) {
                ...
                if ( property_exists( $module, 'includes' ) ) {
                    foreach ( (array) $module->includes as $class ) {
                        $file = WOOLU_PLUGIN_PATH . '/modules/' . $module_slug . '/includes/' . $class . '.php';
                        if ( file_exists( $file ) ) {
                            require_once $file;
                        }
                    }
 ...
 */
function WooLu() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
    $modules = WooLu::instance()->get_modules();
    if ( empty( $modules ) ) {
